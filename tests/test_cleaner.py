...
assert the trimmed text plus the `truncated` flag. Settings are built with
`_env_file=None` so a real `.env` or the dev shell can't bleed the cap in.
"""

from typing import Any

from app.cleaning.cleaner import CleanResult, clean
...
def test_truncates_to_max_content_chars_and_flags_truncated() -> None:
    html = "<html><body>" + ("a" * 50) + "</body></html>"
    result = clean(html, settings=_settings(max_content_chars=20))
    assert len(result.text) == 20
    assert result.text == "a" * 20
    assert result.truncated is True


def test_under_cap_returned_intact_not_truncated() -> None:
...
def test_exact_cap_is_not_truncated() -> None:
    # truncated is len(full) > cap, so text exactly at the cap is NOT flagged.
    html = "<html><body>" + ("a" * 20) + "</body></html>"
    result = clean(html, settings=_settings(max_content_chars=20))
    assert result.text == "a" * 20
    assert result.truncated is False
...
def test_empty_html_returns_empty_string() -> None:
    result = clean("", settings=_settings())
    assert result.text == ""
    assert result.truncated is False
